...
- Added assistant chat with local fallback, local backend support and optional OpenAI browser-session key.
...
- GitHub Pages is static. It cannot run `ai_server.ps1` or store OpenAI/TikTok secrets. The public HTTPS app can use the local backend at `http://127.0.0.1:8789` when it is running, can use a temporary OpenAI key in the browser session, and otherwise falls back to local generation.
